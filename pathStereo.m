...
%     figure(); imagesc(depthMap); axis equal;
    ...
    backwardMap2 = backwardMessage_col_bot2topProp(costMap, sigma, prob);
    theSum = 0;    
for j = 1:10
    diff1 = backwardMap(:,:,j) - backwardMap2(:,:,j); 
%      figure(); imagesc(diff1); axis equal;     
    theSum = theSum + abs(diff1);
end
    fprintf('the sum is %e\n', sum(theSum(:)));
