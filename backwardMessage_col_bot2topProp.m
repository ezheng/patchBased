function backwardMap = backwardMessage_col_bot2topProp(costMap, sigma,prob,NCCDistribution)

backwardMap = zeros( size(costMap) );

tic
parfor col = 1: size(costMap,2)
    backwardMap(:,col,:) = distributionMapComputation_route(costMap, col, sigma, prob,NCCDistribution);
end
fprintf(1, 'SPM computation time: %d seconds\n', toc);
end


function distributionMapACol = distributionMapComputation_route(costMap, col, sigma, prob,NCCDistribution)
...
%         emission_uniform = 0.5;
        emission_uniform = NCCDistribution(floor((1 - costMap(:,col,imageIdx))/ (2/numOfBins))+2);
...
%       compute backward message
        ...
        beta(1,1:2) = 0.5;
        for i = 2:1:height
            beta(i,1) = beta(i-1,1)*emission(i-1) * transitionProb(1,1) + beta(i-1,2)*emission_uniform(i-1) * transitionProb(1,2);
            beta(i,2) = beta(i-1,1)*emission(i-1) * transitionProb(2,1) + beta(i-1,2)*emission_uniform(i-1) * transitionProb(2,2);
            Z = beta(i,1) + beta(i,2);
            beta(i,1) = beta(i,1)/Z;
            beta(i,2) = beta(i,2)/Z;
        end
